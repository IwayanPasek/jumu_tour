...
@section('title', 'Katalog Tempat Wisata Bali - ' . ($brand['name'] ?? 'Bali Tour Service'))
@section('meta_description', 'Jelajahi berbagai pilihan tempat wisata terbaik dan terpopuler di Bali untuk liburan tak terlupakan.')
    ...
    <div class="container">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-2">
                ...
                <li class="breadcrumb-item active text-brand-secondary" aria-current="page">Tempat Wisata</li>
            </ol>
        </nav>
        <h1 class="fw-bold mb-2 display-6">Destinasi & Tempat Wisata di Bali</h1>
        <p class="text-white-50 mb-0 lead fs-6">
            Temukan tempat wisata impian, pantai eksotis, pura bersejarah, hingga panorama alam terbaik.
        </p>
    </div>
</section>
            ...
            <form method="GET" action="{{ route('destinations.index') }}" class="row g-2 align-items-center">
                ...
                <div class="col-md-4 col-lg-5">
                    <div class="input-group">
                        ...
                        <input type="text" 
                               ...
                               placeholder="Cari tempat wisata, pura, pantai..." 
                               value="{{ $search }}">
                    </div>
                </div>
                ...
                <div class="col-md-2 col-lg-2 d-flex gap-2">
                    ...
                    @if($search !== '' || !empty($regionSlug) || !empty($categorySlug))
                        <a href="{{ route('destinations.index') }}" class="btn btn-outline-secondary" title="Reset Filter">
        ...
        @if($search !== '' || !empty($regionSlug) || !empty($categorySlug))
            <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
                <span class="text-muted small">
                    Menampilkan <strong>{{ $destinations->total() }}</strong> destinasi sesuai kriteria pencarian.
                </span>
        ...
        @else
            ...
            <div class="card border-0 shadow-sm rounded-4 p-5 text-center my-4">
                ...
                <h4 class="fw-bold text-dark mb-2">
                    {{ $search !== '' || !empty($regionSlug) || !empty($categorySlug) ? 'Destinasi Tidak Ditemukan' : 'Belum Ada Destinasi Tersedia' }}
                </h4>
                <p class="text-muted small max-w-500 mx-auto mb-4">
                    {{ $search !== '' || !empty($regionSlug) || !empty($categorySlug) 
                        ? 'Tidak ada tempat wisata yang sesuai dengan pencarian Anda. Silakan coba kata kunci atau filter lain.' 
                        : 'Katalog destinasi wisata sedang diperbarui oleh pengelola. Silakan kunjungi kembali nanti.' }}
                </p>
                <div>
                    @if($search !== '' || !empty($regionSlug) || !empty($categorySlug))
